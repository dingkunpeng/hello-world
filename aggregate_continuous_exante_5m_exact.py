...
ROOT=Path('output/continuous_exante_5m_exact')
rows=[]
for p in sorted(ROOT.glob('*/summary.json')):
    j=json.loads(p.read_text(encoding='utf-8'))
    rows.append({'date':j['date'],'trigger_bars':len(j['trigger_bars']),'fills':j['fill_count'],'hedges':j['hedge_group_count'],
                 'capital_usd':j['total_capital_traded_usd'],'net_pnl_usd':j['total_net_pnl_usd'],
                 'positive_hedges':j['positive_hedges'],'negative_hedges':j['negative_hedges'],
                 'uncovered_hedges':j['uncovered_hedges'],'max_hedge_age_ms':j['qa']['max_hedge_book_age_ms']})
summary={
 'policy':'STRICT EX-ANTE 5m stale peg; previous completed 5m Binance index close; static 5m ladder -40/-60/-80/-100/-120bp; $2k each; strict seller-aggressor price-through; +100ms OKX L2 hedge; 32.5bp cost.',
 'days_replayed':len(rows),'trigger_bars':sum(r['trigger_bars'] for r in rows),'fill_count':sum(r['fills'] for r in rows),
 ...
 'positive_days':sum(r['net_pnl_usd']>0 for r in rows),'negative_days':sum(r['net_pnl_usd']<0 for r in rows),'zero_days':sum(abs(r['net_pnl_usd'])<1e-12 for r in rows),
