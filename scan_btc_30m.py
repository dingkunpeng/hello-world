...
START=1767225600000; END_EXCLUSIVE=1788652800000; STEP=30*60*1000
...
def get(url, params=None, tries=6):
    ...
    for i in range(tries):
        try:
            r=requests.get(url, params=params, timeout=45); r.raise_for_status(); return r
        except Exception as e:
            last=e; time.sleep(min(2**i,20))
    raise last

def get_json(url, params): return get(url,params=params).json()

def fetch_spot():
    rows=[]; cur=START; chunk_n=1000
    while cur<END_EXCLUSIVE:
        ...
        if not data: raise RuntimeError(f'empty spot batch at {cur}')
        rows.extend(data); nxt=int(data[-1][0])+STEP
        if nxt<=cur: raise RuntimeError('spot pagination stalled')
        cur=nxt; print('spot',len(rows),datetime.fromtimestamp(data[-1][0]/1000,tz=timezone.utc).isoformat())
    return rows
...
def read_archive_csv(url):
    ...
    with zipfile.ZipFile(io.BytesIO(blob)) as zf:
        ...
        rows=list(csv.reader(zf.read(names[0]).decode('utf-8-sig').splitlines()))
    if rows and rows[0] and not rows[0][0].replace('.','',1).isdigit(): rows=rows[1:]
    return rows
...
def fetch_index():
    ...
    for month in range(1,9):
        ...
        part=read_archive_csv(url); rows.extend(part); print('index month',ym,len(part),'total',len(rows))
    ...
    while t<END_EXCLUSIVE:
        if t not in idx: missing.append(t)
        t+=STEP
    missing_dates=sorted({datetime.fromtimestamp(ms/1000,tz=timezone.utc).strftime('%Y-%m-%d') for ms in missing})
    for ymd in missing_dates:
        url=f'{ARCHIVE}/daily/indexPriceKlines/{SYMBOL}/{INTERVAL}/{SYMBOL}-{INTERVAL}-{ymd}.zip'
        part=read_archive_csv(url)
        for r in part: idx[int(float(r[0]))]=r
        print('index backfill',ymd,len(part),'total unique',len(idx))
...
spot=fetch_spot(); index=fetch_index()
...
keys=sorted(set(spot_map)&set(idx_map)); expected=(END_EXCLUSIVE-START)//STEP
...
events=[]
if candidates:
    chrono=sorted(candidates,key=lambda r:r['open_time_ms']); groups=[]; g=[chrono[0]]
    for r in chrono[1:]:
        if r['open_time_ms']-g[-1]['open_time_ms']<=STEP:g.append(r)
        else:groups.append(g);g=[r]
    groups.append(g)
    for g in groups:
        worst=min(g,key=lambda r:r['deviation_bp'])
        events.append({'start_utc':g[0]['open_time_utc'],'end_utc':iso(g[-1]['open_time_ms']+STEP),'windows':len(g),'min_deviation_bp':worst['deviation_bp'],'worst_window_utc':worst['open_time_utc'],'spot_low_at_worst':worst['spot_low'],'index_low_at_worst':worst['index_low'],'priority':tier(worst['deviation_bp'])})
...
summary={'period_start_utc':'2026-01-01 00:00:00','period_end_inclusive_utc':'2026-09-05 23:59:59','expected_30m_rows':expected,'synchronized_rows':len(keys),'candidate_windows_le_-20bp':len(candidates),'strong_windows_le_-30bp':sum(1 for r in full if r['deviation_bp']<=-30),'extreme_windows_le_-50bp':sum(1 for r in full if r['deviation_bp']<=-50),'candidate_events':len(events),'worst_deviation_bp':min(r['deviation_bp'] for r in full),'top_20_windows':[{k:r[k] for k in ['rank','open_time_utc','spot_low','index_low','deviation_bp','tier']} for r in candidates[:20]],'top_20_events':events[:20]}
