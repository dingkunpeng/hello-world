...
COST_BP=32.5
TICK=0.01
TRIGGER_CSV=Path('output/continuous_exante_5m_fast/potential_trigger_bars.csv')
OUTROOT=Path('output/continuous_exante_5m_exact')
...
def load_bars(date):
    ...
    with TRIGGER_CSV.open(encoding='utf-8-sig') as f:
        for r in csv.DictReader(f):
            if r['date']==date:
                out.append({'bar_utc':r['bar_open_utc'],'start_ms':int(r['bar_open_ms']), 'anchor':float(r['prev_completed_5m_index_close'])})
    return out

def load_binance_trades(date,bars):
    targets=[(b['start_ms'],b['start_ms']+300000) for b in bars]
...
def bar_start(t): return (t//300000)*300000

def replay_day(date):
    bars=load_bars(date)
    if not bars:return None
    bar_map={b['start_ms']:b for b in bars}; targets=set(bar_map)
    trades,burl=load_binance_trades(date,bars); seller=[x for x in trades if x['m']]
    item=okx_item(date)
    armed={(b,off):True for b in targets for off in LADDER_BP}
    fills=[]; pending={}; hedges=[]; ti=0; prev=None; prev_ts=None
    last_needed=max(b+300000 for b in targets)+HEDGE_DELAY_MS+1000

    def schedule(tr):
        b=bar_start(tr['T'])
        if b not in targets:return
        anchor=bar_map[b]['anchor']
        for off in LADDER_BP:
            ...
            if tr['p']<limit:
                ...
                rec={'date':date,'bar_utc':bar_map[b]['bar_utc'],'tier_bp':off,'fill_t_ms':tr['T'],'fill_t_utc':iso(tr['T']),
                     'binance_trade_px':tr['p'],'static_anchor_prev_5m_index_close':anchor,'entry_limit_px':limit,
    ...
    keys=[(x['bar_utc'],x['tier_bp']) for x in fills]
    total_pnl=sum((x['net_pnl_usd'] or 0) for x in hedges); total_cap=sum(x['capital_usd'] for x in hedges)
    summary={'date':date,
      'policy':'STRICT EX-ANTE 5m stale peg: every 5m bar ladder armed from previous completed 5m Binance index close only; static 300s; strict seller-aggressor price-through; +100ms OKX 50-level hedge; 32.5bp cost.',
      'trigger_bars':[b['bar_utc'] for b in bars], 'binance_archive':burl,
      ...
      'uncovered_hedges':sum(1 for x in hedges if x['sell_vwap_usdt'] is None),
      'fills':fills,'hedge_groups':hedges,
      'qa':{'duplicate_bar_tier_fills':len(keys)-len(set(keys)),
    ...
    print(json.dumps({'date':date,'fills':len(fills),'hedges':len(hedges),'cap':total_cap,'pnl':total_pnl,'positive':summary['positive_hedges'],'negative':summary['negative_hedges']},ensure_ascii=False))
    return summary
...
def main():
    ...
    for d in chosen:
        try: replay_day(d)
        except Exception as e: print('ERROR',d,repr(e)); errors.append({'date':d,'error':repr(e)})
    OUTROOT.mkdir(parents=True,exist_ok=True); (OUTROOT/f'shard_{a.shard}_errors.json').write_text(json.dumps(errors,ensure_ascii=False,indent=2),encoding='utf-8')
    if errors: raise RuntimeError(f'{len(errors)} errors')
